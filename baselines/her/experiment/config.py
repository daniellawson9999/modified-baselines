...
def cached_make_env(make_env, pass_env = None):
    """
    ...
    if make_env not in CACHED_ENVS:
        assert (pass_env != None), "must pass environment"
        CACHED_ENVS[make_env] = pass_env
        #env = make_env()
        #CACHED_ENVS[make_env] = env
    return CACHED_ENVS[make_env]


def prepare_params(kwargs, pass_env = None):
    # DDPG params
    ddpg_params = dict()
...
def configure_her(params):
    ...
    #compute_reward = np.vectorize(env.envs[0].compute_reward)
    def reward_fun(ag_2, g, info):  # vectorized
        return env.envs[0].compute_reward(ag_2, g, info)
    
    #reward_fun = np.vectorize(reward_fun)
...
def configure_dims(params):
    ...
    # dims = {
    #     'o': obs['observation'].shape[0], # this gives 1 which should be 12
    #     'u': env.action_space.shape[0],
    #     'g': obs['desired_goal'].shape[0], # this gives 1, which should be 3
    # }
    dims = {
        'o': env.observation_space['observation'].shape[0],
        'u': env.action_space.shape[0],
        'g': env.observation_space['desired_goal'].shape[0]
    }
